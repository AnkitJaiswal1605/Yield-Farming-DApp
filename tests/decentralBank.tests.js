...
contract ('DecentralBank', ([owner, customer]) => {
    ...
    describe('Yield Farming', async () => {
        it('rewards tokens for staking', async () => {
            ...
            assert.equal(result.toString(), tokens('100'), 'decentral bank contract balance after staking');
            ...
            result = await tether.balanceOf(decentralBank.address);
            assert.equal(result.toString(), tokens('0'), 'decentral bank contract balance after unstaking');

            result = await decentralBank.isStaked(customer);
            assert.equal(result.toString(), 'false', 'customer is unstaked');
        })
    })
})